...
import type { Commit } from "grugway/git";
import { createIntegrityTag, updateIntegrityTag } from "grugway/integrity";
import type { Path } from "grugway/shell";
import type { SemVer } from "grugway/version";
import { format } from "grugway/version";
...
function internalUpdateChangelog(
  ...
  const manualHighlightsExist = !(listStart === 0 ||
    listStart >= nextSectionStart);
